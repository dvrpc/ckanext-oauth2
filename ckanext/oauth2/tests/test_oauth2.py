...
class OAuth2PluginTest(unittest.TestCase):
    ...
    def _helper(
        self, fullname_field=True, mail_field=True, conf=None, missing_conf=None, jwt_enable=False
    ):
        oauth2.db = MagicMock()
    ...
    def test_get_token_with_no_credentials(self, oauth2_session_mock):
        state = b64encode(json.dumps({"came_from": "initial-page"}).encode())
        oauth2.toolkit.request = make_request(True, "data.com", "callback", {"state": state})

        helper = self._helper()

        oauth2_session_mock().fetch_token.side_effect = MissingCodeError(
    ...
    def test_get_token(self, OAuth2Session):
        ...
        state = b64encode(json.dumps({"came_from": "initial-page"}).encode())
        oauth2.toolkit.request = make_request(
            True, "data.com", "callback", {"state": state, "code": "code"}
        )
    ...
    def test_get_token_legacy_idm(self, OAuth2Session):
        ...
        retrieved_token = helper.get_token()
        authorization = "Basic " + helper.client_id + ":" + helper.client_secret
        expected_headers = {
            "Accept": "application/json",
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": urlsafe_b64encode(authorization.encode()),
        }
    ...
    def test_get_token_insecure(self):
        ...
        state = b64encode(json.dumps({"came_from": "initial-page"}).encode())
        oauth2.toolkit.request = make_request(
            False, "data.com", "callback", {"state": state, "code": "code"}
        )
    ...
    def test_get_token_invalid_cert(self):
        ...
        with self.assertRaises(InsecureTransportError):
            with patch("ckanext.oauth2.oauth2.OAuth2Session") as oauth2_session_mock:
                oauth2_session_mock().fetch_token.side_effect = SSLError(
    ...
    def test_get_token_error(self):
        helper = self._helper()
        token = {"info": "auth_error", "error_description": "Some description"}
        httpretty.register_uri(httpretty.POST, helper.token_endpoint, body=json.dumps(token))
    ...
    @parameterized.expand(
        [
            ...
            (
                [
                    ("Set-Cookie", 'cookie1="cookie1val"; Path=/'),
                    ("Set-Cookie", 'cookie12="cookie2val"; Path=/'),
                ],
            ),
        ]
    )
    ...
    def test_challenge(self):
        ...
        # Check
        state = urlencode({"state": b64encode(json.dumps({"came_from": came_from}).encode())})
        expected_url = (
    ...
    def test_identify_invalid_token(self, user_info):
        ...
        httpretty.register_uri(
            httpretty.GET, helper.profile_api_url, status=401, body=json.dumps(user_info)
        )
        ...
        try:
            ...
            if user_info["error"] == "invalid_token":
                ...
                self.assertEquals(user_info["error_description"], str(e))
    ...
    @parameterized.expand([({"came_from": "http://localhost/dataset"},), ({},)])
    def test_redirect_from_callback(self, identity):
        came_from = "initial-page"
        state = b64encode(json.dumps({"came_from": came_from}).encode())
        oauth2.toolkit.request = make_request(
            True, "data.com", "callback", {"state": state, "code": "code"}
        )
    ...
    def test_refresh_token(self, user_exists):
        ...
        if user_exists:
            ...
            oauth2.OAuth2Session.assert_called_once_with(
                helper.client_id, token=current_token, scope=helper.scope
            )
            session.refresh_token.assert_called_once_with(
                helper.token_endpoint,
                client_secret=helper.client_secret,
    ...
    def test_refresh_token_invalid_cert(self):
        ...
        with self.assertRaises(InsecureTransportError):
            with patch("ckanext.oauth2.oauth2.OAuth2Session") as oauth2_session_mock:
                oauth2_session_mock().refresh_token.side_effect = SSLError(
                    "(Caused by SSLError(SSLError(\"bad handshake: Error([('SSL routines', 'tls_process_server_certificate', 'certificate verify failed')],)\",),)"
                )
