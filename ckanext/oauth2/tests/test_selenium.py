...
FILAB_PASSWORD = "1234"
PASS_INTEGRATION_TESTS = os.environ.get("INTEGRATION_TEST", "").strip().lower() in (
    "1",
    "true",
    "on",
)
...
class IntegrationTest(unittest.TestCase):
    ...
    def _introduce_log_in_parameters(cls, username=FILAB2_MAIL, password=FILAB_PASSWORD):
        driver = cls.driver
        id_username = WebDriverWait(cls.driver, 10).until(
    ...
    def test_basic_login(self):
        ...
        WebDriverWait(driver, 20).until(
            lambda driver: (self.base_url + "dashboard") == driver.current_url
        )
        self.assertEqual("admin", driver.find_element_by_link_text("admin").text)
        driver.find_element_by_link_text("About").click()
        WebDriverWait(driver, 20).until(
            lambda driver: (self.base_url + "about") == driver.current_url
        )
    ...
    def test_basic_login_different_referer(self):
        ...
        WebDriverWait(driver, 20).until(
            lambda driver: (self.base_url + "dataset") == driver.current_url
        )
        self.assertEqual("admin", driver.find_element_by_css_selector("span.username").text)
    ...
    @parameterized.expand(
        [("user/register", IDM_URL + "/sign_up"), ("user/reset", IDM_URL + "/password/request")]
    )
    def test_register(self, action, expected_url):
        driver = self.driver
        driver.get(self.base_url + action)
        WebDriverWait(driver, 10).until(
            lambda driver: print(driver.current_url) or driver.current_url == expected_url
        )
